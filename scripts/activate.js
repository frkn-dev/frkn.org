...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    let subscription_id = formData.get("id");
    ...
    try {
      if (!subscription_id) {
        msg.textContent = "Создаём пробную подписку...";

        const resTrial = await fetch(`${API_BASE}/trial`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ referred_by: "WEB" }),
        });
        ...
        subscription_id = trialData?.response?.id;

        console.log("Subscription create Response: ", trialData.response);
      ...
      msg.textContent = "Активируем ключ...";
      ...
      if (key) {
        result.innerHTML = `
          ...
          <div style="font-size: small;">
            ID — это твой уникальный идентификатор подписки, рекомендуем сохранить его
          </div>
          <p><b>ID:</b> ${subId}</p>
